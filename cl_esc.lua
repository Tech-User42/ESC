...
AddEventHandler("gyro:setSirenState_c", function(sender, newstate,vehicle)
    ...
    if player_s ~= -1 then 
        ...
        if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
            if IsPedInAnyVehicle(ped_s, false) then
                ...
                if getSirenState(vehicle) == 1 then
                    setSirenState(newstate,veh)
                else
                    setSirenState(newstate,veh)
                end
...
function setSirenState(newstate,veh)
...
function ShowHelpNotification(msg, beep)
	...
	AddTextEntry('ESCHelpNotification', msg)
	DisplayHelpTextThisFrame('ESCHelpNotification', false)
	BeginTextCommandDisplayHelp('ESCHelpNotification')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if _set[entit] then
            if(VehicleSirenState[veh] == nil) then
                VehicleSirenState[veh] = false
            end
            if Config.UI then
            ...
            if IsVehicleSirenOn(veh) then
                ...
                if IsDisabledControlJustPressed(0, CONTROLS['TOGGLE'][2]) then
                    ...
                    VehicleSirenState[veh] = not VehicleSirenState[veh]
                    TriggerServerEvent("gyro:setSirenState_s", VehicleSirenState[veh],veh)
                end
            else
                gyrophare = false
            end
